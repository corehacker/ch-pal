   ...
   PAL_LOGGER_INIT_PARAMS_X *px_init_params)
{
   ...
   if (NULL != px_init_params)
   {
      ...
      {
         (void) snprintf(ca_filename, sizeof(ca_filename), "%s%02d.txt",
            px_init_params->uca_filename_prefix,
            px_init_params->ui_file_name_suffix);
         printf ("Opening File: %s for logging\n", ca_filename);
         gpfp_logger_file_fp = fopen(ca_filename, "w+");
         if (NULL != gpfp_logger_file_fp)
         {
   ...
   ...)
{
         ...
         || (true == gb_enable_console_logging)))
   {
      ...
      (void) vsnprintf (ca_actual_log, sizeof(ca_actual_log), puc_format_str,
         x_va_list);
      va_end (x_va_list);
   ...
   ...)
{
         ...
         || (true == gb_enable_console_logging)))
   {
      ...
      if (true == gb_enable_file_logging && NULL != gpfp_logger_file_fp)
      {
         if (e_level <= ge_level)
         {
            fprintf (gpfp_logger_file_fp, "%10d:0x%X:%s:%4s:%32s:%48s:%4d: %s\n",
               pal_get_system_time (), ui_thread_id,
               ((eLOG_LEVEL_LOW == e_level) ?
                     ("L") :
                     ((eLOG_LEVEL_MEDIUM == e_level) ?
                           ("M") :
