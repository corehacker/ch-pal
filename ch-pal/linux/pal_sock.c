   ...
   *phl_sock_hdl = NULL;
   ...
   if (px_sock_ctxt->hl_sock_hdl < 0)
   {
      PAL_LOG_HIGH("socket failed: %d, Errno: %d",
         px_sock_ctxt->hl_sock_hdl, errno);
   ...
   PAL_SOCK_HDL hl_sock_hdl)
{
   ...
   i_sock_ret = close (px_sock_ctxt->hl_sock_hdl);
   if (0 != i_sock_ret)
   {
      PAL_LOG_HIGH("close failed: %d, Errno: %d", i_sock_ret, errno);
   ...
   PAL_SOCK_ADDR_IN_X *px_sock_addr_in)
{
   ...
   if (0 != i_sock_ret)
   {
      PAL_LOG_HIGH("bind failed: %d, Errno: %d", i_sock_ret, errno);
   ...
   uint32_t ui_backlog)
{
   ...
   if (0 != i_sock_ret)
   {
      PAL_LOG_HIGH("listen failed: %d, Errno: %d", i_sock_ret, errno);
      ...
      if (ePAL_SOCK_CONN_MODE_NON_BLOCKING == e_mode)
      {
         ...
         else
         {
            PAL_LOG_HIGH("connect to %d:%d failed: %d, Errno: %d",
               px_dest_sock_addr_in->x_sin_addr.ui_ip_addr_no,
               px_dest_sock_addr_in->us_sin_port_no, i_sock_ret, errno);
   ...
   PAL_SOCK_IN_ADDR_X *px_in_addr)
{
   PAL_RET_E e_error = ePAL_RET_FAILURE;
   int i_ret_val = -1;
   struct in_addr x_in_addr = {0};

   if ((NULL == puc_ip_addr_str) || (NULL == px_in_addr))
   {
      PAL_LOG_HIGH("Invalid Args");
      e_error = ePAL_RET_INVALID_ARGS;
      goto LBL_CLEANUP;
   ...
   i_ret_val = inet_aton ((const char *) puc_ip_addr_str, &x_in_addr);
   if (0 == i_ret_val)
   {
      PAL_LOG_HIGH("inet_aton failed: %d, Errno: %d", i_ret_val, errno);
      e_error = ePAL_RET_FAILURE;
   }
   else
   {
      px_in_addr->ui_ip_addr_no = x_in_addr.s_addr;
   ...
   PAL_SOCK_IN_ADDR_X *px_sock_in_addr)
{
   ...
   if (NULL == px_host_ent)
   {
      PAL_LOG_HIGH("gethostbyname failed: %p, Errno: %d", px_host_ent, errno);
      e_error = ePAL_RET_INVALID_ARGS;
      goto LBL_CLEANUP;
   }
   ...
   uint32_t ui_flags)
{
   ...
   errno = 0;
   i_sock_ret = recv (px_sock_ctxt->hl_sock_hdl, puc_buf, *pui_buf_len,
      ui_flags);
   if (i_sock_ret < 0)
   {
         ...
         if (ui_no_active_socks > 0)
         {
            ...
            if (true == b_is_set)
            {
               ...
               i_sock_ret = recv (px_sock_ctxt->hl_sock_hdl,
                  (puc_buf + ui_recv_bytes), (*pui_buf_len - ui_recv_bytes),
                  ui_flags);
               if (i_sock_ret < 0)
               {
               ...
               else
               {
                  ...
                  if (i_sock_ret < (*pui_buf_len - ui_recv_bytes))
                  {
   ...
   uint32_t ui_flags)
{
   ...
   errno = 0;
   ui_flags |= MSG_NOSIGNAL;
   i_sock_ret = send (px_sock_ctxt->hl_sock_hdl, puc_buf, *pui_buf_len,
      ui_flags);
   if (i_sock_ret < 0)
   {
         ...
         if (ui_no_active_socks > 0)
         {
            ...
            if (true == b_is_set)
            {
               ...
               i_sock_ret = send (px_sock_ctxt->hl_sock_hdl,
                  (puc_buf + ui_sent_bytes), (*pui_buf_len - ui_sent_bytes),
                  ui_flags);
               if (i_sock_ret < 0)
               {
                  e_error = ePAL_RET_SOCK_SEND_FAILED;
               ...
               else
               {
                  ...
                  if (i_sock_ret < (*pui_buf_len - ui_sent_bytes))
                  {
                     ui_curr_time_ms = pal_get_system_time();
                     ui_elapsed_time_ms = ui_curr_time_ms - ui_start_time_ms;
                     if (ui_elapsed_time_ms < ui_timeout_ms)
   ...
   uint32_t ui_hostname_len)
{
   ...
   if (i_ret_val < 0)
   {
      PAL_LOG_HIGH("gethostname failed: %d, Errno: %d", i_ret_val, errno);
   ...
   PAL_SOCK_ADDR_IN_X *px_dest_sock_addr_in)
{
   ...
   i_sendto_ret = sendto (px_sock_ctxt->hl_sock_hdl, puc_buf, ui_buf_len,
      ui_flags, (const struct sockaddr *) &x_sock_addr_in,
      sizeof(x_sock_addr_in));
   if (i_sendto_ret <= 0)
   {
      PAL_LOG_HIGH("sendto failed: %d, Errno: %d", i_sendto_ret, errno);
      *pui_buf_len = 0;
      e_error = ePAL_RET_SOCK_SEND_FAILED;
   }
   else
...
{
   PAL_RET_E e_error = ePAL_RET_FAILURE;
   ssize_t i_recvfrom_ret = -1;
   PAL_SOCK_CTXT_X *px_sock_ctxt = NULL;
   uint32_t ui_buf_len = 0;
   struct sockaddr_in x_sock_addr_in   = {0};
   socklen_t ui_addr_len = 0;
   ...
   px_sock_ctxt = (PAL_SOCK_CTXT_X *) hl_sock_hdl;
   ui_addr_len = sizeof(x_sock_addr_in);
   ui_buf_len = *pui_buf_len;
   i_recvfrom_ret = recvfrom (px_sock_ctxt->hl_sock_hdl, puc_buf, ui_buf_len,
      ui_flags, (struct sockaddr *) &x_sock_addr_in, &ui_addr_len);
   if ((i_recvfrom_ret <= 0) || (0 == ui_addr_len))
   {
      PAL_LOG_HIGH("recvfrom failed: %d, Errno: %d", i_recvfrom_ret, errno);
      *pui_buf_len = 0;
      e_error = ePAL_RET_SOCK_RECV_FAILED;
   }
   else
...
{
   PAL_RET_E e_error = ePAL_RET_FAILURE;
   struct addrinfo *px_addrinfo_list = NULL;
   struct addrinfo *px_addrinfo = NULL;
   uint32_t ui_list_count = 0;
   PAL_SOCK_IN_ADDR_X *px_in_addr = NULL;
   struct sockaddr_in *px_sockaddr_in = NULL;
   int i_error = -1;

   if (NULL == puc_hostname_str)
   {
      PAL_LOG_HIGH("Invalid Args");
      e_error = ePAL_RET_FAILURE;
      goto LBL_CLEANUP;
   ...
   errno = 0;
   i_error = getaddrinfo ((const char *) puc_hostname_str, NULL, NULL,
      &px_addrinfo_list);
   if ((0 != i_error) || (NULL == px_addrinfo_list))
   {
      PAL_LOG_HIGH("getaddrinfo failed: %d, errno: %d", i_error, errno);
      PAL_LOG_HIGH("Error in getaddrinfo: %s", gai_strerror(i_error));
      e_error = ePAL_RET_SOCK_ADDR_RESOLUTION_FAILED;
      goto LBL_CLEANUP;
   }

   for (px_addrinfo = px_addrinfo_list; px_addrinfo != NULL;
         px_addrinfo = px_addrinfo->ai_next, ui_list_count++);
   px_addr_list->ppx_addr_list = pal_malloc (
      sizeof(PAL_SOCK_IN_ADDR_X *) * ui_list_count, NULL);
   if (NULL == px_addr_list->ppx_addr_list)
   {
      PAL_LOG_HIGH("pal_malloc failed");
      e_error = ePAL_RET_OUT_OF_MEMORY;
      goto LBL_CLEANUP;
   }

   /* loop over all returned results and do inverse
    * lookup */
   for (px_addrinfo = px_addrinfo_list; px_addrinfo != NULL ; px_addrinfo =
      px_addrinfo->ai_next)
   {
      px_sockaddr_in = (struct sockaddr_in *) px_addrinfo->ai_addr;
      px_addr_list->ppx_addr_list[px_addr_list->ui_count] = pal_malloc (
         sizeof(PAL_SOCK_IN_ADDR_X), NULL);
      if (NULL == px_addr_list->ppx_addr_list[px_addr_list->ui_count])
      {
         PAL_LOG_HIGH("pal_malloc failed");
         break;
      }
      px_in_addr = px_addr_list->ppx_addr_list[px_addr_list->ui_count];
      px_in_addr->ui_ip_addr_no = px_sockaddr_in->sin_addr.s_addr;
      px_addr_list->ui_count++;
   }
   e_error = ePAL_RET_SUCCESS;
LBL_CLEANUP:
   if (NULL != px_addrinfo_list)
   {
      freeaddrinfo(px_addrinfo_list);
      px_addrinfo_list = NULL;
   }
   return e_error;
}
