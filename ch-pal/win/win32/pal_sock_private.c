   ...
   uint32_t ui_data_len)
{
   ...
   i_sock_ret = setsockopt(px_sock_ctxt->hl_sock_hdl, SOL_SOCKET, SO_REUSEADDR,
      (const char *) &(px_data->ui_opt_val), sizeof(px_data->ui_opt_val));
   if (0 != i_sock_ret)
   {
      PAL_LOG_HIGH("setsockopt failed: %d, Errno: %d", i_sock_ret, WSAGetLastError());
   ...
   uint32_t ui_data_len)
{
   ...
   i_sock_ret = setsockopt (px_sock_ctxt->hl_sock_hdl, IPPROTO_TCP,
      SO_KEEPALIVE, (const char *) &(px_data->ui_keepalive_time_ms),
      ...
      else
      {
         ...
         if (1 == i_fd_isset)
         {
            ...
            i_sockopt_val = getsockopt (px_sock_ctxt->hl_sock_hdl, SOL_SOCKET,
               SO_ERROR, (char *) &i_opt_val, &i_opt_len);
            if (i_sockopt_val < 0)
            {
               PAL_LOG_HIGH("getsockopt failed: %d, Errno: %d",
                  i_sockopt_val, WSAGetLastError());
               e_error = ePAL_RET_SOCK_CONNECT_FAILED;
               break;
            }
            if (1 == i_opt_val)
