...
/*********************************** MACROS ***********************************/
#define PAL_FILE_MAX_LINE_SIZE                  (10)
   ...
   const uint8_t *puc_open_mode)
{
   ...
   px_f_ctxt->hl_file_hdl = fopen ((const char *) puc_file_path,
   ...
   uint32_t *pui_actual_line_len)
{
   ...
   uint8_t *puc_temp = 0;
   int i_feof_ret = -1;
   bool b_buffer_overflow = false;
   uint8_t uc_one_byte = 0;
   ...
   do
   {
      if (false == b_buffer_overflow)
      {
         puc_temp = &(puc_buf [ui_count]);
      }
      else
      {
         puc_temp = &uc_one_byte;
      }
      ui_fread_ret = fread (puc_temp, 1, 1, px_f_ctxt->hl_file_hdl);
      if (1 == ui_fread_ret)
      {
   ...
   {
      PAL_LOG_HIGH("Buffer overflow occured. Supplied %d bytes, but the "
         "line is more than %d bytes: %d. Truncating!", ui_buf_len, ui_count,
         ui_actual_line_len);
      puc_buf [ui_count - 1] = '\0';
      *pui_line_len = ui_count;
      *pui_actual_line_len = ui_actual_line_len;
      e_error = ePAL_RET_FILE_READ_BUF_OVERFLOW;
   }
   else
   {
   ...
   uint32_t *pui_buf_len)
{
   ...
   {
      PAL_LOG_HIGH("fwrite failed: %d, Errno: %d. Reqd: %d, Read: %d, Desc: %p",
         ui_fwrite_ret, errno, ui_buf_len, ui_fwrite_ret, px_f_ctxt->hl_file_hdl);
...
{
   PAL_RET_E e_error = ePAL_RET_FAILURE;
   uint8_t uca_line [PAL_FILE_MAX_LINE_SIZE] = { 0 };
   uint32_t ui_line_len = 0;
   uint32_t ui_line_count = 0;
   uint32_t ui_actual_line_count = 0;

   if ((NULL == hl_file_hdl) || (NULL == pui_line_count))
   {
      PAL_LOG_HIGH("Invalid Args");
      e_error = ePAL_RET_INVALID_ARGS;
      goto LBL_CLEANUP;
   ...
   while (1)
   {
      e_error = pal_freadline_v2 (hl_file_hdl, uca_line, sizeof(uca_line),
         &ui_line_len, &ui_actual_line_count);
      if (ePAL_RET_FILE_EOF_REACHED == e_error)
      {
         e_error = ePAL_RET_SUCCESS;
         break;
      }
      ...
      {
         if (ePAL_RET_FILE_READ_BUF_OVERFLOW == e_error)
         {
            ui_line_count++;
            continue;
         }
         else if (ePAL_RET_SUCCESS != e_error)
         {
            PAL_LOG_HIGH("pal_freadline failed: %d", e_error);
            break;
         }
         else
         {
